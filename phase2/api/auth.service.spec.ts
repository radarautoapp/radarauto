 ...
 * AuthService tests
 *
 * Cobertura mínima obrigatória (Regra 27):
 *  - Hash de senha + verificação
 *  - Erro genérico em login inválido (sem vazar se email existe)
 *  - Sessão multi-device (não invalida outras ao logar)
 *  - validateSession rejeita revogada/expirada/user desativado
 *  - Funcionario só pode ser criado por lojista (Regra 10)
 */
...
const buildSession = (
  overrides: Partial<{ id: string; userId: string; revokedAt: Date | null; expiresAt: Date }> = {},
) => ({
  id: "session-1",
  userId: "user-1",
  ipAddress: null,
...
describe("AuthService", () => {
  ...
  describe("login", () => {
    ...
    it("rejeita com mesma mensagem quando senha errada (não vaza que email existe)", async () => {
  ...
  // Silencia warning de tipo
  let _suppressTs: Tx<void>;
  beforeAll(() => {
    _suppressTs = Promise.resolve();
  });
});
