/**
 * EditStorePhoneModal — edita telefone OU WhatsApp da loja com OTP.
 *
 * Mesma estrutura do EditPhoneModal do user, mas:
 *  - Salva no Store, não no User
 *  - Aceita prop `kind: 'phone' | 'whatsapp'` pra escolher qual campo
 *  - Sucesso difere por kind
 */
"use client";
...
type Step = "input" | "otp";
type Kind = "phone" | "whatsapp";

const LABELS: Record<
  Kind,
  { fieldLabel: string; modalTitle: string; modalDesc: string; success: string }
> = {
  phone: {
    fieldLabel: "Telefone comercial",
    modalTitle: "Editar telefone da loja",
    modalDesc: "Vamos enviar um código pra confirmar o novo número.",
    success: "Telefone da loja atualizado!",
  },
  whatsapp: {
    fieldLabel: "WhatsApp comercial",
    modalTitle: "Editar WhatsApp da loja",
    modalDesc: "Vamos enviar um código pra confirmar o novo número.",
    success: "WhatsApp da loja atualizado!",
  },
};

export interface EditStorePhoneModalProps {
  open: boolean;
  kind: Kind;
...
export function EditStorePhoneModal({
  ...
  onClose,
  onUpdated,
}: EditStorePhoneModalProps): JSX.Element {
  const labels = LABELS[kind];

  const [value, setValue] = useState(maskPhone(currentValue ?? ""));
  ...
  useEffect(() => {
    if (open) {
      setValue(maskPhone(currentValue ?? ""));
      setStep("input");
      setError(null);
      setSuccess(null);
      setBusy(false);
  ...
  const originalDigits = (currentValue ?? "").replace(/\D/g, "");
  const newDigits = value.replace(/\D/g, "");
  const changed = newDigits !== originalDigits;
  const isValid = newDigits.length >= 10;

  // SMS desabilitado: salva o numero direto, sem verificacao por codigo.
  ...
  const goToOtp = async (): Promise<void> => {
    ...
    setBusy(true);
    try {
      const payload = kind === "phone" ? { phone: newDigits } : { whatsapp: newDigits };
      await storesApi.updateMine(payload);
      onUpdated();
      setSuccess({
  ...
  const onVerified = async (token: string): Promise<void> => {
    ...
    try {
      const payload =
        kind === "phone"
          ? { phone: newDigits, phoneVerificationToken: token }
          : { whatsapp: newDigits, whatsappVerificationToken: token };
      await storesApi.updateMine(payload);
      onUpdated();
      setSuccess({
        title: labels.success,
  ...
  return (
    <EditFieldModal
      open={open}
      title={step === "input" ? labels.modalTitle : "Confirme o código"}
      description={
        step === "input" ? labels.modalDesc : `Enviamos um SMS pra ${maskPhone(newDigits)}.`
      }
      onClose={onClose}
